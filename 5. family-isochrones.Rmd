...
So, I'm going to run every 100th family. At 30 families, it will take about 10 mins.

```{r}
fam_isochrones <- list()
for (row in seq(1, nrow(fams_as_pts_lvprop), by=100)) {
  ...
  fam_isochrones[[row]] <- fam_iso
  print(row)
}

fam_isochrones_df = do.call(rbind, fam_isochrones)
```

```{r}
fam_isos <- st_set_geometry(fam_isochrones_df, NULL)
fam_AdjSup <- merge(fams_as_pts_lvprop, fam_isos, by.x = 'FID', by.y = 'FID', all.x = TRUE)
fam_AdjSup_samp <- na.omit(fam_AdjSup)
```

```{r}
write_sf(obj=fam_AdjSup_samp, dsn="Shapefiles/fam_AdjSup_samp.shp")
```
