...
```{r select-daycares-within-one-mile-of-lakeview}
...
tm_shape(st_buffer(lakeview, 5280*1.5)) + tm_borders() +
  ...
  tm_shape(prov_isochrones_df) + tm_polygons(alpha=0.25) +
  tm_shape(daycare_lakeview) + tm_dots(col="darkgreen", size=0.25) 
```
...
```{r add-SPR-to-point-df}
prov_isochrones_df <- st_set_geometry(prov_isochrones_df, NULL)
daycare_SPR <- merge(daycare_lakeview, prov_isochrones_df, 
